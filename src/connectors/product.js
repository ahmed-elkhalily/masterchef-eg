...
export default function(data) {
	...
	const getOne = async function(data) {
		...
		const res = await callServer(
			url + 'api/v1/product/details/' + data.id,
			requestOptions
		)
		return res.data
	}
	...
	const searchProducts = async(data) => {
		...
		if (data.min_price) {
			query === '?'
				? (query = query + 'min_price=' + data.min_price)
				: (query = query + '&min_price=' + data.min_price)
		}
		if (data.max_price) {
			query === '?'
				? (query = query + 'max_price=' + data.max_price)
				: (query = query + '&max_price=' + data.max_price)
		}
		const res = await callServer(
	...
	const productReviews = async(data) => {
		...
		const res = await callServer(
			url + 'api/v1/product/reviews/' + data.id,
			requestOptions
		)
		return res.data
	}
