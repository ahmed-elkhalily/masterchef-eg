...
<template>
	<v-container>
		<v-col cols="12" md="12" class="d-flex flex-wrap ma-auto py-md-12">
			<v-col cols="12" md="3" class="pt-10">
				<h3 class="text-center text-md-start py-4">
					{{ $t('userBoard.title') }}
				</h3>
				...
				>
					...
					<v-tab value="addresses">{{
						$t('userBoard.tabs.addressName')
					}}</v-tab>
					<v-tab value="accountDetails">{{
						$t('userBoard.tabs.accountDetailsName')
					}}</v-tab>
					<v-tab value="wishlist">{{
						$t('userBoard.tabs.wishlistName')
					}}</v-tab>
					<v-tab @click="authStore.logOut(), router.push({ path: '/' })">{{
						$t('userBoard.tabs.logoutName')
					}}</v-tab>
				</v-tabs>
